    ...
    <div id="register">
        <div class="container">
            <section class="step-1 mt-40 mb-40">
                ...
                <p class="mt-5">
                    Lo invitamos a completar la siguiente información corporativa. Una vez termine de completar los campos,
                ...
                <form method="post" action="{{ route('company.complete_info.save') }}">
                    ...
                    <div class="row d-flex group mt-4">
                        ...
                        <div class="col-12 col-md-6 mt-3">
                            <label>Nombre *</label>
                            <input type="text" name="contact_person" value="{{ $company->contact_person }}" required
                                style="text-transform: uppercase;" />
                        </div>
                        <div class="col-12 col-md-6 mt-3">
                            <label>Cargo *</label>
                            <select id="list_contacto_position" required onchange="actualizarNombreCargoContacto(this);">
                                ...
                                @foreach ($listaCargos as $cargo)
                                    <option @if ($cargo->vinculoCargoTITULO == $company->contact_position) selected @endif
                                        value="{{ $cargo->vinculoCargoTITULO }}">{{ $cargo->vinculoCargoTITULO }}</option>
                                @endforeach
                            </select>
                            <input type="text" id="contact_position" name="contact_position"
                                value="{{ $company->contact_position }}" required
                                style="text-transform: uppercase;display:none;" />
                        </div>
                        <div class="col-12 col-md-6 mt-3">
                            <label>Email *</label>
                            <input type="text" name="contact_email" value="{{ $company->registration_email }}"
                                required />
                        </div>
                        <div class="col-12 col-md-6 mt-3">
                            <label>Celular *</label>
                        ...
                        <div class="col-12 col-md-6">
                            <div class="form-group mb-3">
                                ...
                                <select name="contact_sexo" required>
                                    <option value="">SELECCIONE UNO</option>
                                    <option value="MASCULINO" @if ($company->contact_sexo == 'MASCULINO') selected @endif>MASCULINO
                                    </option>
                                    <option value="FEMENINO" @if ($company->contact_sexo == 'FEMENINO') selected @endif>FEMENINO
                                    </option>
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="cuadro_datos_usuarios " style=" ">
                        ...
                        <p style="text-align:center">Nombre de usuario:&nbsp;
                            <strong>{{ $company->registration_email }}</strong></p>
                        <h3 style="text-align:center">Por seguridad Le recomendamos cambiar la contrase&ntilde;a cuando
                            ingrese .</h3>
                    </div>
...
@section('js')
    <script>
        ...
        $('document').ready(function() {
            ...
            $("#sector").val('{{ $company->sector_id ?? ($company->ciiuActividad->macroSectorID ?? 0) }}');

            initselect('/municipios/listado', {{ $company->department_id ?? 0 }}, '#municipality',
                '{{ $company->municipality_id }}');

            @if ($company->ciiuActividad()->first() != null)
                initselect('/secciones/listado',
                    {{ $company->sector_id ?? ($company->ciiuActividad->macroSectorID ?? 0) }}, '#seccion',
                    '{{ $company->ciiuActividad->ciiuSeccionID }}');
                initselect('/actividades/listado', {{ $company->ciiuActividad->ciiuSeccionID ?? 0 }}, '#actividad',
                    '{{ $company->ciiuActividad->ciiuactividad_id }}');
            @endif
